...
SCRIPT="$REPO_DIR/NSL3130_driver/src/roboscan_nsl3130/scripts/intrinsic_calibration_node.py"
...
if [ -z "$CAMERA_ID" ]; then
    ...
    if [ -n "$CAMERA_ID" ]; then
        ...
        echo "            Connect the camera or pass as 4th argument: ./intrinsic_calib.sh 8x13 0.04 /topic N00A5060D"
        CAMERA_ID="nsl"
    fi
fi
