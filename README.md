...
> **토픽 네임스페이스(기본 동작)**: 토픽은 기본적으로 **장치 ID(카메라 시리얼)로 네임스페이스가 붙어** `/{device_id}/...` 로 나옵니다(예: `/N0078060D/point_cloud`). 별도 인자 없이 그대로 멀티 Edge로 확장됩니다(6장 참고). rviz 설정도 자동으로 맞춰집니다.
> - 네임스페이스 없이(`/point_cloud`, `/rgb/image_raw` …) 띄우려면 `namespace:=''` 를 주세요.
> - 시리얼이 안 잡히면 자동으로 네임스페이스 없이 동작합니다.
...
**퍼블리시 토픽:** (기본 네임스페이스 적용 시 `{device_id}` = 카메라 시리얼. `namespace:=''` 면 접두어 없이 `/rgb/image_raw` 등)

| 토픽 | 타입 | 설명 |
|------|------|------|
| `/{device_id}/rgb/image_raw` | `sensor_msgs/Image` | RGB 이미지 |
| `/{device_id}/depth/image_raw` | `sensor_msgs/Image` | 거리 이미지 |
| `/{device_id}/point_cloud` | `sensor_msgs/PointCloud2` | XYZI 포인트클라우드 |
| `/{device_id}/point_cloud_rgb` | `sensor_msgs/PointCloud2` | XYZRGB (캘리브레이션 완료 후 활성화) |
...
> **네임스페이스 (자동)**: intrinsic/extrinsic 런치도 **USB 시리얼을 자동감지해 토픽 네임스페이스를 카메라와 똑같이 맞춥니다**(1 Edge–1 카메라 전제). 따라서 카메라를 기본값(`namespace:=auto`)으로 띄웠다면 캘리브레이션도 **추가 인자 없이** 그대로 돌리면 됩니다.
...
- `/{device_id}/point_cloud_rgb` — 캘리브레이션 결과 기반 XYZRGB 포인트클라우드
...
- 모든 머신은 **같은 `ROS_DOMAIN_ID=42` 한 그래프**를 공유하고, Edge는 **장치 ID(카메라 시리얼) 네임스페이스**로 토픽을 분리합니다(`namespace:=auto` 가 기본값). Host는 모든 Set의 토픽을 바로 구독할 수 있습니다.

### 6-1. 충돌 방지 규칙

| 항목 | 규칙 | 예시 |
|------|------|------|
| 토픽 | 기본 `namespace:=auto` → `/{device_id}/...` 자동 분리 (device_id = 시리얼) | `/N0078060D/point_cloud` |
| 노드 | 네임스페이스 하위로 자동 | `/N0078060D/roboscan_publish_node` |
| TF frame | 시리얼 기반 자동 (이미 유니크) | `N0078060D_lidar_frame` / `N0078060D_camera_frame` |
| DDS 도메인 | **42 공통** (모든 머신) | — |
...
**LAN IP 할당 예시** (중복만 없으면 숫자는 자유):

| 머신 | LAN IP |
|------|--------|
| Host | `192.168.0.10` |
| 1 Set (Edge) | `192.168.0.51` |
| 2 Set (Edge) | `192.168.0.52` |
| 3 Set (Edge) | `192.168.0.53` |
...
- `namespace` 는 **기본이 `auto`** 라 아무 인자 없이도 시리얼을 읽어 `/{device_id}/...` 로 퍼블리시합니다(시리얼 미검출 시 자동으로 네임스페이스 없이 동작).
- rviz/rqt는 Edge에선 끄고(`use_rviz:=false use_rqt:=false`), 아래처럼 Host에서 봅니다.

### 6-4. Host 에서 보기

```bash
ros2 topic list | grep camera/point_cloud                        # 살아 있는 카메라(device_id) 확인
ros2 topic hz   /N0078060D/point_cloud                    # 수신 확인 (device_id 예시)
rviz2   # PointCloud2 디스플레이 토픽을 /{device_id}/point_cloud 로 지정
```

| 보고 싶은 것 | 토픽 |
|---|---|
| 포인트클라우드 | `/{device_id}/point_cloud` |
| XYZRGB | `/{device_id}/point_cloud_rgb` |
| 거리/RGB 이미지 | `/{device_id}/depth/image_raw` · `/{device_id}/rgb/image_raw` |
...
5. 그냥 실행만 하면(`namespace` 기본 `auto`) → `/{device_id}/...` 자동 생성. 끝.
...
- **토픽이 접두어 없이(`/point_cloud` 등) 겹친다** → 어떤 Edge를 `namespace:=''` 로 띄운 것(기본 `auto` 면 `/{device_id}/...` 라 안 겹침). 일부러 끈 게 아니면 인자를 빼고 다시 띄우세요.
